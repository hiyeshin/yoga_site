...
def register(request):
	"""
	When a rpi connets to the WS server this is called
	"""
	if request.method == 'POST':
		...
		rpi_db, created = RaspberryPi.objects.get_or_create(mac_address = jreq['mac'], defaults = {'current_ip':jreq['ip'], 'name':'%s - %s' % (jreq['mac'], jreq['ip'])})
