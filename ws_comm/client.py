...
def push_config(rpi):
	rpi_read_ifaces = RPIReadInterface.objectsl.filter(rpi = rpi)
	rpi_write_iface = RPIWriteInterface.objects.filter(rpi = rpi)
...
def config_changed_signal(sender, **kwargs):
	...
	if sender in read_display_cls or sender in write_display_cls:
		# then send update request for the relavant rpi config
