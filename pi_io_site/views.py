from django.http imporpt HttpRespose
...
def rpi_displays(request, rpi_mac):
	...
	rpi = RaspberryPi.obejcts.get(pk = rpi_mac)
	...
	read_and_write_display = read_display + write_displays
	...
	for display in read_and_write_displays:
		for instance in display['instances']:
			...
			if key not in data_bindings_json:
				data_bindings_json[key] = {display['cls_name'].lower():{'ids':[instance['ids']]}}
			else:
